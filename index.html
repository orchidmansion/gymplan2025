...
<head>
    ...
    <title>Your Personalized Gym Regime</title>
            ...
            const generateImage = async (prompt) => {
                try {
                    ...
                    if (result.predictions && result.predictions.length > 0 && result.predictions[0].bytesBase64Encoded) {
                        ...
                        throw new Error("Image generation failed or unexpected response structure.");
                    }
                } catch (err) {
                    console.error("Error generating image:", err);
                    return `https://placehold.co/400x200/FFCCCC/CC0000?text=Image+Failed+for+${encodeURIComponent(prompt)}`;
                }
            };
            ...
            const ExerciseCard = ({ exercise, onToggleComplete, isCompleted }) => {
                ...
                useEffect(() => {
                    ...
                    const fetchImage = async () => {
                        setLoading(true);
                        if (isMounted) {
                            const url = await generateImage(exercise.imagePrompt);
                            setImageUrl(url);
                            setLoading(false);
                        }
                    };
                        ...
                        <div className="flex items-center justify-between mb-2">
                            <span className="flex items-center text-lg font-semibold text-gray-800">
                                <svg className="h-6 w-6 text-blue-500 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    <path d="M12 14V2m-5 7H11a4 4 0 0 1 4 4v3a2 2 0 0 0 2 2h0a2 2 0 0 0 2-2v-3a4 4 0 0 1 4-4h4"></path><path d="M15 14a4 4 0 0 0 4 4v3a2 2 0 0 0 2 2h0a2 2 0 0 0 2-2v-3a4 4 0 0 0 4-4"></path>
                                </svg>
            ...
            return (
                <div className="min-h-screen bg-gray-100 p-4 sm:p-6 md:p-8">
                    <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-xl p-6 sm:p-8 lg:p-10">
                        <h1 className="text-3xl sm:text-4xl font-bold text-center text-gray-800 mb-6 lg:mb-10">Your Personalized Gym Regime</h1>
